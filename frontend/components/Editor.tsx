...
export default function Editor() {
  ...
  return (
    <div className="h-full flex flex-col bg-white relative">
          ...
          <div className="relative">
            ...
            {showMenu && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border border-slate-100 z-20 py-1">
                  <button 
      ...
      {/* Formatting Bar */}
      <div className="flex items-center px-6 py-2 border-b border-slate-50 space-x-1 overflow-x-auto no-scrollbar">
      ...
      <div className="flex-1 relative">
        <textarea
          ...
          className="w-full h-full p-6 resize-none focus:outline-none text-slate-800 leading-relaxed text-lg font-light"
          spellCheck={false}
        />

        {/* Tag Autocomplete Popup */}
        {showTagSuggestions && filteredTags.length > 0 && (
          <div className="absolute bottom-4 left-6 bg-white rounded-lg shadow-xl border border-slate-200 p-2 z-30 max-h-48 overflow-y-auto min-w-[200px]">
            <div className="text-xs font-semibold text-slate-400 px-2 py-1 mb-1">SUGGESTED TAGS</div>
            {filteredTags.map(tag => (
              <button
                key={tag.name}
                onClick={() => insertTag(tag.name)}
                className="w-full text-left px-2 py-1.5 text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-600 rounded flex items-center justify-between"
              >
                <span>#{tag.name}</span>
